...
The generated `collector-state` branch stores an encrypted manifest and encrypted file chunks. Saves use the validated prior Git tree as their base and send only changed encrypted files and explicit removals. Unchanged chunks and the separate authentication envelope are inherited without rebuilding the full file list. This avoids the confirmed GitHub tree-creation processing timeout that occurred when a request listed 1,565 files for only four changes. Commits remain parentless and concurrent-writer checks remain in force. The state key is separate from the dashboard key because collector caches include complete source threads. Only an explicit file allowlist is transferred; source credentials, Ryu notes, logs, locks, and process files are excluded. Restores validate authenticated ciphertext, file hashes, sizes, and paths. Plaintext exists only in the runner's private working directory and is removed at the end of the job. No plaintext Actions caches or artifacts are uploaded.

The workflow runs only trusted `main` code, pins its setup actions, serializes runs, and uses GitHub's job-scoped token to publish within this repository and read its Actions history. Successful source updates can publish even if another source fails; failed sources retain their previous evidence. Every prepared dashboard snapshot is checked for decryption, including after partial source failures. State progress is saved after both successful and partial runs.

Refresh errors remain visible as step warnings and in the job's health summary. The overall run becomes a failure to trigger existing GitHub Actions notifications when attention is needed:

- State restoration, dashboard publication, or collector-progress saving: the third consecutive affected refresh, or 30 minutes of continuous failure, whichever is reached first.
- Task/activity evidence at least 30 minutes old, Slack evidence at least 60 minutes old, or module evidence at least three hours old: immediate notification. Both snapshot dates and the collectors' last complete-success dates are checked; partial progress cannot make incomplete collection look fresh.
- Failed dashboard decryption/integrity: immediate notification.
- An already reported continuous incident: warning-only runs with reminders every six hours. A healthy observation resets that condition; a different newly affected condition is evaluated independently.

Successful fixed health-marker steps store only condition and observation booleans in Actions run history, independently of the collector-state branch. The monitor reads trusted `main` runs of this workflow and verifies actual notification-step failure before considering an alert sent. Skipped/unavailable checks preserve prior incidents; only an observed healthy check resets them. Unknown intervals do not count as failed attempts or prove 30 minutes of continuous failure. Verification/probe runs do not reset incidents. If history cannot be verified, the monitor alerts visibly instead of silently suppressing a problem. A retained source-error flag during its retry cooldown no longer counts as a new failed collection attempt. A successful overall run can therefore include documented warnings; it does not certify every source is current. No account-wide email settings are changed.

Collector-state metadata reads and saves retry temporary server/network failures up to four total attempts, with a 60-second request timeout and a 120-second total wait budget per request. Rate-limit retries honor GitHub's cooldown; longer cooldowns stop the attempt instead of retrying too early. Authentication, permission and validation errors fail immediately. Reference-write retries first check the branch: an already-applied write counts as success, while a changed branch is preserved. Public diagnostics report only fixed operation names, HTTP status, error category and retry timing, never raw responses or private content. Saving collector state happens after dashboard publication; its failures are tracked separately by the health policy above.
