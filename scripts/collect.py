...
def collect(config, live=False):
    board=Path(config['completion'])
    stale=Path(config['staleness'])
    raw_html=(board/'board.html').read_text()
    match=re.search(r'<script type="application/json" id="seed">(.*?)</script>',raw_html,re.S)
    if not match: raise ValueError('Completion seed not found')
    seed=json.loads(match.group(1))
    rows=load(stale/'rows.json')
    status=load(stale/'statuses.json')['statuses']
    users=load(stale/'users.json')
    dues=load(stale/'deadlines.json')
    slack=load(stale/'slack_activity.json')
    annotations=load(stale/'annotations.json')
    history=[json.loads(line) for line in (stale/'history.jsonl').read_text().splitlines() if line.strip()][-600:]
    activity_at=None  # Legacy render time does not prove when Studio rows were read.
    ...
    confirmations_doc=load(board/'confirmations.json')
    ...
    def normalize_confirmation(name):
        ...
        return {'state':rec.get('state') or 'not_checked','asked':stamp(rec.get('asked_at')), 'reply':stamp(rec.get('reply_at')), **{k:rec.get(k) for k in ('asked_by','asked_url','deadline','writer','reply_by','reply_text','reply_url')}}
    bridge_module=None
    if live:
        spec=importlib.util.spec_from_file_location('existing_completion_adapter',board/'refresh_board.py')
        ...
        try:
            ...
            activity_at=task_at=iso(datetime.now(UTC))
        finally:
            if bridge_module._bridge: bridge_module._bridge.close()
    ...
    tasks=[]; conflicts=[]
    root_map=load(board/'ee_thread_roots.json')['roots']
    def domain_of(name):
    ...
    for r in rows:
        ...
        if not old and not live: continue
        ...
        t=old[1] if old else {}
        newer_completion=not live and parse(task_at)>parse(activity_at) if activity_at else not live
        sid=t.get('status',r['status']) if newer_completion else r['status']
        ...
        moved=t.get('moved',r.get('transitioned_at')) if newer_completion else r.get('transitioned_at')
        owner_id=t.get('owner') if newer_completion and 'owner' in t else r.get('owned_by')
        conflict=bool(newer_completion and (sid!=r['status'] or moved!=r.get('transitioned_at') or owner_id!=r.get('owned_by')))
        if conflict: conflicts.append(tid)
        ...
        side=dues.get(name,{})
        root=root_map.get(re.sub(r'-TEMPLATE$','',name))
        ...
        tasks.append({'id':tid,'name':name,'cohort':cohort,'artifact':r.get('artifact'),'domain':t.get('domain') or domain_of(name),
          ...
          'updated_at':r.get('updated_at'),'transitioned_at':moved,'activity_conflict':conflict or not live,'blocked':str(r.get('blocked')).lower()=='true',
    ...
    modules=load(config['modules']);validate_modules(modules)
    ...
    return {'schema_version':1,'generated_at':now,'tasks':tasks,'modules':modules,
      ...
      'confirmations':{'at':confirmation_at,'cadence_minutes':None,'label':'Slack confirmations (manual harvest)'},
      'slack':{'at':max((s.get('seen_at') or s.get('at') for s in slack.values()),default=None),'cadence_minutes':None,'label':'Slack activity (partial coverage)'},
      'modules':{'at':modules['generated_at_utc'],'cadence_minutes':60,'label':'Module failure aggregates'}},
      'history':history,'conflicts':conflicts,'mode':'live Studio read + existing sidecars' if live else 'existing local snapshots'}
